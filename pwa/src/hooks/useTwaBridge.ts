...
type TwaBridge = {
    ...
    subscribe: (fn: (data: any, ev: MessageEvent) => void, options?: { replay?: boolean }) => () => void;
 ...
 * 
 */
export function useTwaBridge(onMessage: (data: any, ev: MessageEvent) => TwaBridge | void) {
    ...
    useEffect(() => {
        ...
        const wrapper: (data: any, ev: MessageEvent) => void = (data, ev) => {
            try { latest.current?.(data, ev); } catch (e) { console.error(e); }
        };
            ...
            case 'nbl:set-alarms':
                ...
                */
                ...
                const results = alarms.map((alarm: any) => {
                    if (alarm.mode === "exact") {
                        return { id: alarm.id, mode: alarm.mode, ok: false, reason: "past_time" };
                    } else {
                ...
                channel.port2.postMessage(JSON.stringify(
                    {
                        ...
                        apps: [
                            ...
                            { packageName: "com.google.android.deskclock", label: "Google 鬧鐘" },
                        ],
