...
export function EditDialog<T>({
  ...
  const [openCronPreviewField, setOpenCronPreviewField] = useState<
    string | null
  >(null);
  //為了避免同樣的item來亂
      ...
      if (!!nextRunField && !!cronField && !!!isRunning) {
        ...
        const currentNextRun =
          item && typeof item === "object"
            ? (item as Record<string, any>).nextRun
            : undefined;
        const predictedNextRun = cronExpr
          ? getPredictedNextRun(cronExpr, new Date())
          : undefined;
        ...
        if (
          ...
          nextRunField &&
          cronExpr &&
          predictedNextRun != null &&
          currentNextRun !== predictedNextRun
        ) {
  ...
  const renderCronField = (field: DialogField) => {
    ...
    const previewRuns =
      openCronPreviewField === field.name
        ? getUpcomingOccurrences(buildCronExpr(parts))
        : [];
  ...
  return (
    ...
    >
      ...
      >
        ...
        {error && (
          <div className="mb-4 p-3 bg-red-50 border border-red-200 text-red-600 rounded text-sm shrink-0">
            {error}
          </div>
        )}
                ...
                <input
                  ...
                  value={
                    typeof formData[field.name] === "number"
                      ? formatToDateTimeLocal(formData[field.name])
                      : (formData[field.name] ?? "")
                  }
                  onChange={(e) => handleChange(field.name, e.target.value)}
        ...
        <div className="flex items-center justify-between gap-3 shrink-0 pt-4 border-t">
          ...
          <div className="flex gap-3 justify-end">
            <button
              onClick={() => onClose(false)}
              onTouchEnd={(event) =>
                handleDialogActionTouchEnd(event, () => onClose(false))
              }
              disabled={isSaving}
            ...
            <button
              ...
              onTouchEnd={(event) =>
                handleDialogActionTouchEnd(event, handleSave)
              }
              disabled={isSaving}
