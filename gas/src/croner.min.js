// 此為 [croner.min.js](./gas/src/croner.min.js) 所下載來使用
// 此為 [Croner](https://github.com/Hexagon/croner/tree/master) 專案所擁有，版權歸原作者所有
...
minitz.toTZ = function (d, tzStr) {
  const localDateString = d
    .toLocaleString("en-US", { timeZone: tzStr })
    .replace(/[\u202f]/, " ");
...
minitz.tp = (y, m, d, h, i, s, tz) => {
  return { y: y, m: m, d: d, h: h, i: i, s: s, tz: tz };
};
...
function CronOptions(options) {
  ...
  delete options.name;
  options.legacyMode =
    options.legacyMode === void 0 ? true : options.legacyMode;
  options.paused = options.paused === void 0 ? false : options.paused;
  options.maxRuns = options.maxRuns === void 0 ? Infinity : options.maxRuns;
  options.catch = options.catch === void 0 ? false : options.catch;
  options.interval =
    options.interval === void 0 ? 0 : parseInt(options.interval, 10);
  options.utcOffset =
    options.utcOffset === void 0 ? void 0 : parseInt(options.utcOffset, 10);
...
CronPattern.prototype.parse = function () {
  if (
    !(typeof this.pattern === "string" || this.pattern.constructor === String)
  ) {
...
CronPattern.prototype.handleNumber = function (
  conf,
  type,
  valueIndexOffset,
  defaultValue,
  ...
  if (isNaN(i)) {
    throw new TypeError(
      "CronPattern: " + type + " is not a number: '" + conf + "'",
    );
  }
...
CronPattern.prototype.setPart = function (part, index, value) {
  ...
  if (part === "dayOfWeek") {
    ...
    if ((index < 0 || index > 6) && index !== "L") {
      throw new RangeError(
        "CronPattern: Invalid value for dayOfWeek: " + index,
      );
    }
  ...
  if (part === "second" || part === "minute") {
    if (index < 0 || index >= 60) {
      throw new RangeError(
        "CronPattern: Invalid value for " + part + ": " + index,
      );
    }
  } else if (part === "hour") {
    if (index < 0 || index >= 24) {
...
CronPattern.prototype.handleRangeWithStepping = function (
  conf,
  type,
  valueIndexOffset,
  defaultValue,
  ...
  if (matches === null)
    throw new TypeError(
      "CronPattern: Syntax error, illegal range with stepping: '" + conf + "'",
    );
  ...
  if (lower > upper)
    throw new TypeError(
      "CronPattern: From value is larger than to value: '" + conf + "'",
    );
...
CronPattern.prototype.handleRange = function (
  conf,
  type,
  valueIndexOffset,
  defaultValue,
  ...
  if (split.length !== 2) {
    throw new TypeError(
      "CronPattern: Syntax error, illegal range: '" + conf + "'",
    );
  }
...
CronPattern.prototype.handleStepping = function (
  conf,
  type,
  valueIndexOffset,
  defaultValue,
  ...
  if (split.length !== 2) {
    throw new TypeError(
      "CronPattern: Syntax error, illegal stepping: '" + conf + "'",
    );
  }
  let start = 0;
  if (split[0] !== "*") {
  ...
  if (steps > this[type].length)
    throw new TypeError(
      "CronPattern: Syntax error, max steps for part is (" +
        this[type].length +
        ")",
    );
...
CronPattern.prototype.setNthWeekdayOfMonth = function (index, nthWeekday) {
  if (nthWeekday === "L") {
    ...
    this["dayOfWeek"][index] =
      this["dayOfWeek"][index] | OCCURRENCE_BITMASKS[nthWeekday - 1];
  ...
  if (pattern.lastDayOfMonth) {
    if (this.month !== 1) {
      ...
      lastDayOfMonth = new Date(
        Date.UTC(this.year, this.month + 1, 0, 0, 0, 0, 0),
      ).getUTCDate();
    }
  }
  const fDomWeekDay =
    !pattern.starDOW && target == "day"
  ...
  for (let i = this[target] + offset; i < pattern[target].length; i++) {
    ...
    if (target === "day" && !pattern.starDOW) {
      ...
      if (dowMatch && dowMatch & ANY_OCCURRENCE) {
        dowMatch = this.isNthWeekdayOfMonth(
          this.year,
          this.month,
          i - offset,
          dowMatch,
        ...
        throw new Error(
          `CronDate: Invalid value for dayOfWeek encountered. ${dowMatch}`,
        );
      }
...
function Cron(pattern, fnOrOptions1, fnOrOptions2) {
  ...
  if (
    ...
    (pattern instanceof Date ||
      (typeof pattern === "string" && pattern.indexOf(":") > 0))
  ) {
    this._states.once = new CronDate(
      pattern,
...
Cron.prototype.schedule = function (func) {
  ...
  this._states.currentTimeout = setTimeout(
    () => this._checkTrigger(target),
    waitMs,
  );
...
Cron.prototype._next = function (prev) {
  ...
  if (
    ...
    prev &&
    prev.getTime() < this.options.startAt.getTime()
  ) {
  ...
  if (this.options.startAt.getTime() <= now.getTime()) {
    ...
    while (prevTimePlusInterval <= now.getTime()) {
      prev = new CronDate(
        prev,
        this.options.timezone || this.options.utcOffset,
      ).increment(this._states.pattern, this.options, true);
