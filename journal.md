...
2. [syncUtils.ts](pwa\src\utils\syncUtils.ts) 補上對 ics_sources 與 ics_events 的處理，確保在 applyChange 中正確處理 sourceId 與 eventId。
...
- [14:18] 設法解決沒有instance，按widget 不會跳到要的與出現網址列的問題
...
強制重新驗證 assetlinks，指令如下：
``` ps1
# 1. 重設驗證狀態
adb shell pm set-app-links --package com.yescirculation.nonblockinglife 0 all
...
- [17:47] 可以由 scheduled -> alarm_queue，然而，mergedAlarmQueueItems 仍然有些問題，還需要再 debug
...
- [21:08] 無法 Deploy ，Rerun 也會出錯，可能GitHub 塞車，晚一點再試
```sh
...
## [2026-07-02] 第一位使用者 AJL，謝謝他提供的一些發現，已修改

## [2026-07-01] 準備請親朋好友幫我測試

## [2026-07-01] 連拍兩個影片，也修改相應的頁面的說明

## [2026-06-30] 改用 GlobalDialog 來取代 confirm來打開 url，試看看iPhone 接不接受
...
## [2026-06-29] 改成讓多個 inputDialog 參數只需要打開一個 dialog 即可。

## [2026-06-29] 將 Macro 改成一格格的方塊

## [2026-06-28] 加入讓使用者可以知道現在的版本與GAS上面的不符，得去更新GAS的

## [2026-06-27] 試著加入macro的功能，下一個commit 來做，有這個，就可以連動了

- 目前做到第五步

## [2026-06-26] 讓 GuidePage 裡面的影片可以依照語系來顯示，並且加上 Android 版的展示影片和 vocus
...
- 加上 id ，免得 TWA出問題
- 透過 [LauncherActivity.java](twa\app\src\main\java\com\yescirculation\nonblockinglife\LauncherActivity.java) 的 `getLaunchingUrl` 來實現`分享`
...
原本的 [NBL_Timer.flo](android\NBL_Timer.flo) 不能用，得手動輸入，還有，新版的可能不吃JSON字串，所以，得改
...
## [2026-05-20] 加了一個在結束任務時，可以選擇是否要結束後編輯他的功能，因為我們可能會想要做些修改，此外，英文的同步部分準備開拍
...
## [2026-04-08] 在 SelectionCachePage 選擇`只記錄` ，最後，應該refresh 才對，這樣才會有正確的列表

順便拍影片和聲音，明天再組成影片

## [2026-04-06] 教學輪播全部完成，接下來是影片拍攝與剪輯了！

## [2026-04-05] 教學輪播的第三頁完成，明天第四頁

## [2026-04-04] 教學輪播的第二頁完成

## [2026-04-03] 第一頁動畫化，內容還在調整中

## [2026-04-03] 加入 iPhone 上的 `NBL Last Log Time` 的功能與說明，這樣就可以在打開社交媒體時自動呼叫NBL，提醒自己先看看待辦清單再決定要不要打開了！
...
1. NBL使用者有可能Safari切到私密瀏覽，就會導致沒辦法使用D B紀錄
2. 開始應該要多一個紀錄的按鈕，因為有時候只想記錄事件，卻不想要記錄開始到結束的時間
